...
@pendency_bp.route("/pendency", methods=["GET"])
@jwt_required
...
@pendency_bp.route("/pendency/<pendency_id>", methods=["GET"])
@jwt_required
...
@pendency_bp.route("/bill/<bill_id>/pendency", methods=["GET"])
@jwt_required
def get_bill_pendencies(bill_id):
    ...
    if error:
        return jsonify(error), http_status_for_service_error(error)
    return jsonify({"pendency": result}), 200


@pendency_bp.route("/group/<group_id>/pendency", methods=["GET"])
@jwt_required
...
@pendency_bp.route("/pendency/<pendency_id>/debtor-confirmation", methods=["PUT"])
@jwt_required
...
@pendency_bp.route("/pendency/<pendency_id>/creditor-confirmation", methods=["PUT"])
@jwt_required
