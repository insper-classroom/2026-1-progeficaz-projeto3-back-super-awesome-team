...
def test_login_invalid_credentials(mock_service, client):
    ...
    response = client.post(
        "/auth/sessions", json={"email": "test@example.com", "password": "wrong"}
    )
...
def test_verify_email_ok(mock_service, client):
    ...
    response = client.get("/auth/email-verifications/valid-token")
    assert response.status_code == 200
...
def test_verify_email_invalid_token(mock_service, client):
    ...
    response = client.get("/auth/email-verifications/bad-token")
...
def test_forgot_password_error(mock_service, client):
    ...
    response = client.post("/auth/password-resets", json={})
